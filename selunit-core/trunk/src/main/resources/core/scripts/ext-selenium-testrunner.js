...
HtmlRunnerTestLoop.prototype.initialize=function(htmlTestCase, metrics, seleniumCommandFactory) {
	//<--START-CHANGES--
	LOG.info('Start test case "'+htmlTestCase.pathname+'" at: '+(new Date().getTime()));
	//--END-CHANGES-->
...
HtmlTestRunner.prototype.runNextTest=function () {
	//<--START-CHANGES--
	if (this.currentTest) {
		LOG.info('Finished test case "'+testFrame.getCurrentTestCase().pathname+'" at: '+(new Date().getTime()));		
	}
	//--END-CHANGES-->	
