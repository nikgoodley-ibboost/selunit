...
HtmlRunnerTestLoop.prototype._recordFailure = function(errorMsg) {
	...
	// <--START-CHANGES--
    if (Selenium.capabilities.failFast) {
	    LOG.error("Fail fast activation aborted test execution due to errors");
		if (htmlTestRunner.currentTest) {
			LOG.info('Finished test case "'
					+ testFrame.getCurrentTestCase().pathname + '" at: '
					+ (new Date().getTime()));
			htmlTestRunner.runAllTests=false;
			htmlTestRunner.currentTest.testComplete();
		}
		htmlTestRunner.getTestSuite().getCurrentRow().saveTestResults();
	    new SeleniumTestResult(htmlTestRunner.getTestSuite().failed, htmlTestRunner.getTestSuite().getTestTable()).post();
	    if (htmlTestRunner.currentTest) {
	    	htmlTestRunner.currentTest.pause();
	    }
    }
	// --END-CHANGES-->
}
